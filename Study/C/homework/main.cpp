// main.cpp承担了所有的展示工作，但代码仍分散在各个类中
// 抱歉，不会写mfc或者其他gui，展示部分在命令行中进行

#include <iostream>
#include "Queue.h"
#include "Graph.h"
//仅有生成哈夫曼编码的代码在main.cpp中
...
void printProject(const char* sen) {
    cout << "----------------------------------" << endl;
    cout << sen << endl;
    cout << "----------------------------------" << endl;
}
int main1()
{
	...
	//第一项演示开始
    printProject("实验一 线性链表及其相关操作");
	Linear::showOff();//其实这样封装已经没有任何意义了...
    PAUSE CLS
    //第二项开始
    printProject("实验二 队列及其相关操作");
    Queue::showOff();
    PAUSE CLS
    //第三项开始
    printProject("实验三 赫夫曼编码");
    int w[6] = { 15, 21, 18, 5, 5, 8};
	int n = 6;
	HuffmanTree myTree;
	char** treeCode;
    ...
    //第四项开始
    //使用ppt中例子
    printProject("实验四 图的邻接表存储与深度优先遍历");
    Graph g;
    g.printGraph();
    g.DFSTraverse();
    PAUSE CLS
    int an, vn;
    cout << "自定义图，请输入顶点数与边数，中间用空格隔开：";
    cin >> vn >> an;
    Graph m(vn, an);
    m.printGraph();
    m.DFSTraverse();
