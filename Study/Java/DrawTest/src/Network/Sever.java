        ...
        while (!exit) {
            ...
            if (protocol.equals("sync")) {
                try {
                    ...
                    toolName = comm.substring(index + 1);
                    // 输出流
...
/*                    do{
    ...
    void sendMessage(int cho) {
        try {
            ...
            //         stop$id
            if (cho == 1) {
                myPS.println("stop$" + ID);
            } else {
                myPS.println("join$" + ID + "$" + name);
            }
            myPS.flush();
            Thread.sleep(100);
        } catch (Exception e1) {
    ...
    void sendImage() {
        ...
        try {
            ...
            //         join$id$你的昵称
            //         stop$id
            myPS.println("sync$" + ID + "$" + fis.available() + "$" + toolName);
            myPS.flush();
            Thread.sleep(100);
