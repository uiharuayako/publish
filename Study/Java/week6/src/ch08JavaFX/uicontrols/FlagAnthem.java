...
public class FlagAnthem extends Application {
	...
	public void start(Stage primaryStage) {
		...
		// 获取当前路径 String mediaFileBase = getClass().getResource("").getPath();
		String mediaFileBase = System.getProperty("user.dir").replace('\\', '/');
		System.out.println(mediaFileBase);

		// Load images and audio
		for (int i = 0; i < NUMBER_OF_NATIONS; i++) {
			images[i] = new Image("file:///" + mediaFileBase + "/bin/image/flag" + i + ".gif");
			mp[i] = new MediaPlayer(new Media("file:///" + mediaFileBase + "/bin/audio/anthem/anthem" + i + ".mp3"));
		}
